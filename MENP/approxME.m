function [Cp,Cm,CQe,CQm,Csum] = approxME(x,y,z,f,Ex,Ey,Ez,n_x,n_y,n_z)
%APPROXME Multipole Expansion under long-wavelength approximation
% Multipole expansion is calculated from current density distribution.
% [ref.1,Table1]
%
% Input properties
...
% T. Hinamoto (Kobe University, Japan)
    ...
    px = -1./(1i*omega).*(trapz4Dto1D(Jx,x,y,z)+ ...
         k.^2/10.*trapz4Dto1D(dpx,x,y,z));
    py = -1./(1i*omega).*(trapz4Dto1D(Jy,x,y,z)+ ...
         k.^2/10.*trapz4Dto1D(dpy,x,y,z));
    pz = -1./(1i*omega).*(trapz4Dto1D(Jz,x,y,z)+ ...
         k.^2/10.*trapz4Dto1D(dpz,x,y,z));
